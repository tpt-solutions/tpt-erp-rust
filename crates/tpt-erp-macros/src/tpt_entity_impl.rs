...
//!
//! Implemented against the runtime traits in `tpt-erp-entity`. The generated code
//! references the `tpt_erp_entity` crate, so the consumer must depend on it.
//!
//! ```ignore
                ...
                Kind::String => checks.push(quote! {
                    if self.#ident.trim().is_empty() {
                        return ::core::result::Result::Err(
                            ::tpt_erp_entity::ValidationError::Required(#name));
                    }
                }),
                ...
                if kind == Kind::String {
                    checks.push(quote! {
                        if !self.#ident.contains('@') || !self.#ident.contains('.') {
                            return ::core::result::Result::Err(
                                ::tpt_erp_entity::ValidationError::Email(#name));
                        }
                    });
                }
            }
                    ...
                    checks.push(quote! {
                        {
                            ...
                            if len < #min || len > #max {
                                return ::core::result::Result::Err(
                                    ::tpt_erp_entity::ValidationError::Length(#name, len, #min, #max));
                            }
                        }
                    });
                }
                ...
                checks.push(quote! {
                    {
                        ...
                        if !in_range {
                            return ::core::result::Result::Err(
                                ::tpt_erp_entity::ValidationError::Range(
                                    #name, v.to_string(),
...
pub(crate) fn derive(input: DeriveInput) -> Result<TokenStream> {
    ...
    let auditable_impl = if let (Some(ca), Some(ua), Some(cb), Some(ub)) = (
        ...
        quote! {
            impl ::tpt_erp_entity::Auditable for #entity {
                fn audit_fields(&self) -> ::core::option::Option<::tpt_erp_entity::AuditFields> {
                    ::core::option::Option::Some(::tpt_erp_entity::AuditFields {
                        created_at: self.#ca,
                        updated_at: self.#ua,
    ...
    let expanded = quote! {
        impl ::tpt_erp_entity::EntityTable for #entity {
        ...
        impl ::tpt_erp_entity::Validatable for #entity {
            #[allow(clippy::manual_range_contains)]
            fn validate(&self) -> ::core::result::Result<(), ::tpt_erp_entity::ValidationError> {
                #(#validate_body)*
        ...
        impl ::tpt_erp_entity::Filter for #filter_name {}

        impl ::tpt_erp_entity::ApplyFilter<#entity> for #filter_name {
