...
//! `DELETE /:id` to a [`Repository`](tpt_entity::Repository), honouring
//! pagination + filtering and invoking an [`AuthPolicy`](tpt_entity::AuthPolicy)
...
pub(crate) fn derive(input: DeriveInput) -> Result<TokenStream> {
    ...
    let mut auth_ty: Type = syn::parse_quote!(::tpt_entity::AllowAll);
    ...
    let expanded = quote! {
        ...
        struct #state_name<R: ::tpt_entity::Repository<#entity_ty> + 'static, A: ::tpt_entity::AuthPolicy> {
            repo: ::std::sync::Arc<R>,
            _auth: ::core::marker::PhantomData<A>,
        }

        impl<R: ::tpt_entity::Repository<#entity_ty> + 'static, A: ::tpt_entity::AuthPolicy> ::core::clone::Clone
            for #state_name<R, A>
        {
            fn clone(&self) -> Self {
        ...
        impl ::core::convert::From<::tpt_entity::RepositoryError> for #error_name {
            fn from(e: ::tpt_entity::RepositoryError) -> Self {
                match e {
                    ::tpt_entity::RepositoryError::NotFound => #error_name::NotFound,
                    ::tpt_entity::RepositoryError::Validation(v) => #error_name::BadRequest(v.to_string()),
                    ::tpt_entity::RepositoryError::Conflict(m) => #error_name::Conflict(m),
                    ::tpt_entity::RepositoryError::Backend(m) => #error_name::Internal(m),
                }
            }
        }
        ...
        impl #api {
            ...
            where
                R: ::tpt_entity::Repository<#entity_ty> + 'static,
                A: ::tpt_entity::AuthPolicy,
            {
                ::axum::Router::new()
                    ...
                    .layer(::axum::extract::Extension(::tpt_entity::Principal::default()))
        ...
        async fn #list<R, A>(
            ...
            filter: ::axum::extract::Query<<#entity_ty as ::tpt_entity::EntityTable>::Filter>,
            principal: ::axum::extract::Extension<::tpt_entity::Principal>,
        ) -> ::core::result::Result<::axum::Json<::tpt_entity::Page<#entity_ty>>, #error_name>
        where
            R: ::tpt_entity::Repository<#entity_ty> + 'static,
            A: ::tpt_entity::AuthPolicy,
        {
            let principal = &principal.0;
            A::authorize(::tpt_entity::Operation::List, principal)
                .map_err(|_| #error_name::Forbidden)?;
            let pagination = ::tpt_entity::Pagination {
        ...
        async fn #get_one<R, A>(
            state: ::axum::extract::State<#state_name<R, A>>,
            id: ::axum::extract::Path<<#entity_ty as ::tpt_entity::EntityTable>::Id>,
            principal: ::axum::extract::Extension<::tpt_entity::Principal>,
        ...
        {
            let principal = &principal.0;
            A::authorize(::tpt_entity::Operation::Read, principal)
                .map_err(|_| #error_name::Forbidden)?;
        ...
        {
            let principal = &principal.0;
            A::authorize(::tpt_entity::Operation::Create, principal)
                .map_err(|_| #error_name::Forbidden)?;
        ...
        {
            let principal = &principal.0;
            A::authorize(::tpt_entity::Operation::Update, principal)
                .map_err(|_| #error_name::Forbidden)?;
        ...
        {
            let principal = &principal.0;
            A::authorize(::tpt_entity::Operation::Delete, principal)
                .map_err(|_| #error_name::Forbidden)?;
