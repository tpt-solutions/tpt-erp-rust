...
//! `cargo build -p tpt-cli` + `tpt plugin build`). If the component is absent the
//! test is skipped, so a clean checkout without the wasm32 target still passes CI.

use std::path::PathBuf;
use std::sync::Arc;

use tpt_wasm::host::{HostContext, TptHost};
use tpt_wasm::{Money, PluginRuntime, RuntimeError, RuntimeConfig};
...
fn wasm_path() -> Option<PathBuf> {
    ...
    // crates/tpt-wasm -> repo root -> examples/plugins/pricing/pricing.wasm
    let p = base
        .parent()?
        .parent()?
